...
void MoveablePoly :: draw() {
   ...
   // ofLogNotice("fColour : ") << fColour;
    ...
    ofFloatColor sColour = selected ? strokeColourSelected : strokeColour;
    if(dimmed) sColour*=0.5;
    else if(isDisabled) sColour*=0.7;
    //ofLogNotice("sColour : ") << sColour;
...
DragHandle* MoveablePoly :: getMainDragHandle() {
    if((mainDragHandleIndex<0) || (mainDragHandleIndex>=handles.size())) {
        return nullptr;
    } else {
...
void MoveablePoly::setHue(int hue) {
    ...
    updateHandleColours();
   // ofLogNotice() << "MoveablePoly::setHue - fillColourSelected : " << fillColourSelected << " fillColour : " << fillColour;
...
void MoveablePoly::resetColours() {
   ...
   // ofLogNotice() << "MoveablePoly::resetColours - fillColourSelected : " << fillColourSelected << " fillColour : " << fillColour;
...
bool MoveablePoly :: setFromPoints(vector<glm::vec2>* points) {
    ...
    if(mainDragHandleIndex>=handles.size()) {
        mainDragHandleIndex = -1;
    }
