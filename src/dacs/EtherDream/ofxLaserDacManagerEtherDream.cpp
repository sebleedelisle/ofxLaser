...
void DacManagerEtherDream :: threadedFunction() {
    ...
    while(ofGetElapsedTimeMillis() < startUpDelayMS) {
        sleep(50);
    }
    ...
    while(isThreadRunning()) {
        ...
        do {
            ...
            if(numBytesReceived >=36)  {
                ...
                double now = static_cast<double>(ofGetElapsedTimeMillis()) / 1000.0;
...
//                            cout << "Point count      :" << status.point_count << endl;
        ...
        if(isThreadRunning() && lock()) {
            ...
            lastCheckTime = static_cast<double>(ofGetElapsedTimeMillis()) / 1000.0;
            unlock();
        }
