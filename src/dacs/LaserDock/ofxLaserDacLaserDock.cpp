...
DacLaserDock:: ~DacLaserDock() {
    ...
    cout << ("DacLaserDock:: ~DacLaserDock()") << endl;
}
void DacLaserDock :: close() {
    
    cout << ("DacLaserDock:: close()") << endl;
...
//    }
    ...
    for(int i = 0; i<numpointstosend; i++) {
        ...
        if(bufferedPoints.size()>0) {
            ...
            dacPoint.x = ofMap(armed ? laserPoint.x : 400, 0, 800, LASERDOCK_MIN, LASERDOCK_MAX); // X is flipped!
    ...
    if(success) {
        lastDataSentTimeMicros = sendTime;
        
        lastAckTimeMicros = ofGetElapsedTimeMicros();
        lastReportedBufferFullness = lastDataSentBufferSize = dacCommand.numPoints;
        
        stateRecorder.recordStateThreadSafe(lastDataSentTimeMicros, 1, lastReportedBufferFullness, lastAckTimeMicros-lastDataSentTimeMicros, dacCommand.numPoints, pps, dacCommand.size());
    } else {
        connected = false; 
