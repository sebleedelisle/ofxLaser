...
bool ShapeTarget :: addShape(std::shared_ptr<Shape> shapetoadd, bool useClipRectangle, ofRectangle clipRectangle){
...
bool ShapeTarget :: addShapes(vector<std::shared_ptr<Shape>> shapestoadd){

    bool changed = false;
    for(std::shared_ptr<Shape>& shape : shapestoadd) {
...
bool ShapeTarget :: setBounds(const ofRectangle& boundsrect){
...
void ShapeTarget ::  processShapes() {
    ...
    vector<std::shared_ptr<Shape>> emptyShapes;
    ...
    vector<vector<std::shared_ptr<Shape>>> sortedShapeContainers;
    ...
    if(anyShapesFilled) {
        ...
        for(int i = 0; i<shapes.size(); i++) {
            ...
            if(shape->id==-1) {
                ...
                for(vector<std::shared_ptr<Shape>>& shapecontainer : sortedShapeContainers) {
        ...
        std::sort(sortedShapeContainers.begin(), sortedShapeContainers.end(), [](const vector<std::shared_ptr<Shape>>& a, const vector<std::shared_ptr<Shape>>& b) -> bool {
            ...
            if((int)(d1*1000) == (int)(d2*1000)) return false;
            else return d1<d2;
        });
    ...
    if(anyShapesFilled) {
        ...
        ofxLaserClipper::Paths clipperMaskPaths;
        
        // go from front to back
        for(int i = sortedShapeContainers.size()-1; i>=0; i--) {
            vector<std::shared_ptr<Shape>>& shapeContainer = sortedShapeContainers[i];
            
            for(std::shared_ptr<Shape>& shape : shapeContainer)  {
                if(shape->isStroked() && (!shape->isEmpty())) {
                    // clip the shape to the current mask, if we have one
                    vector<std::shared_ptr<Shape>> clippedShapes = ClipperUtils::clipShapeToMask(shape, clipperMaskPaths);
