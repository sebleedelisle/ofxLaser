...
void ofxLaser :: setup (int width, int height) {
...
void ofxLaser :: connectButtonPressed(){
...
void ofxLaser :: drawShapes() {
	
	ofPoint currentPosition = laserHomePosition;
	vector<ofxLaserPoint> shapepoints;
	...
	if(shapes.size()>0) {
		...
		shapes.push_front(new LaserDot(currentPosition, c, 0, 0));
		...
		do {
			...
			LaserShape& shape1 = *shapes[currentIndex];
	...
	while(laserPoints.size()<minPoints) {
		...
		LaserCircle blank(laserHomePosition + ofPoint(0,10), 10, ofFloatColor(0), 2,2,0);
		shapepoints.clear();
...
void ofxLaser :: renderPreview() {
	...
	deque<LaserShape*>& newshapes = shapes; // (shapesHistory.size()>0) ? shapesHistory.back() : shapes;
...
void ofxLaser :: addLaserDot(const ofPoint& ofpoint, ofFloatColor colour, float dotintensity, int maxpoints){
	
	// TODO re-implement 3D to 2D conversion
	//shapes.push_back(new LaserDot(gLProject(ofpoint), colour, intensity));
	shapes.push_back(new LaserDot(ofpoint, colour, dotintensity, (maxpoints<0) ? (int)dotMaxPoints : maxpoints));
	
}

// TODO allow specification of acceleration and speed

void ofxLaser:: addLaserLine(const ofPoint&startpoint, const ofPoint&endpoint, ofFloatColor colour, float speed, float acceleration) {
	shapes.push_back(new LaserLine(startpoint, endpoint, colour, speed<0 ? (float)defaultLineSpeed : speed, acceleration<0 ? (float) defaultLineAcceleration : acceleration));
}

// TODO allow specification of acceleration, speed, and overlap
void ofxLaser :: addLaserCircle(const ofPoint& centre, float radius, ofFloatColor colour, float speed, float acceleration, float overlap){
	
	//ofPoint p = gLProject(ofpoint);
	//float scalar = gLGetScaleForZ(ofpoint.z);
	shapes.push_back(new LaserCircle(centre, radius, colour, speed<0 ? (float)defaultCircleSpeed : speed, acceleration<0 ? (float)defaultCircleAcceleration : acceleration, overlap<0 ? (float)defaultCircleOverlap : overlap));
}


// TODO implement multicolour, and allow custom acceleration and speed

void ofxLaser::addLaserPolyline(const ofPolyline& line, ofColor colour, float speed, float acceleration, float cornerthreshold){
...
//	}
	...
	shapes.push_back(new LaserPolyline(line, colour, acceleration< 0 ? (float) defaultPolylineAcceleration : acceleration, speed<0 ? (float) defaultPolylineSpeed : speed, cornerthreshold<0 ? (float) defaultPolylineCornerThreshold : cornerthreshold));
	
	
}


void ofxLaser::drawTestPattern() {
//	
	if(testPattern==1) {
		...
		addLaserCircle(maskRectangle.getCenter(), 10, ofColor::white);
		addLaserCircle(maskRectangle.getCenter(), 50, ofFloatColor(1,0,0));
...
void ofxLaser :: addPointsForMoveTo(const ofPoint & currentPosition, const ofPoint & targetpoint){
...
void ofxLaser :: addPoint(ofPoint p, ofFloatColor c, float pointIntensity, bool useCalibration) {

	
	addPoint(ofxLaserPoint(p, c, pointIntensity, useCalibration));
	
}


void ofxLaser :: addPoints(vector<ofxLaserPoint>&points) {
...
void ofxLaser :: addPoint(ofxLaserPoint p) {
...
//
//void ofxLaser :: addIldaPoint(ofPoint p, ofFloatColor c, float pointIntensity, bool useCalibration){
//	
//	
...
void  ofxLaser :: processIldaPoints() {
...
//
//void ofxLaser :: addIldaPointRaw(ofPoint p, ofFloatColor c, bool useCalibration) {
//	
////	if(useCalibration) {
...
//
//
//ofxIlda::Point ofxLaser :: ofPointToIldaPoint(const ofPoint& ofpoint, ofFloatColor colour){
//	
//	ofPoint p = ofpoint;
//	
...
//}
//
//ofPoint ofxLaser :: ildaPointToOfPoint(const ofxIlda::Point& ildapoint){
//	
...
void ofxLaser:: connectToEtherdream() {
	
	etherdream.setup();
	//shouldBeConnected = true;
	//etherdream.setPPS(10000);
}
void ofxLaser:: disconnectFromEtherdream() {
...
//void ofxLaser :: updateRenderDelay() {
//	
...
//	warp.dragSpeed = 1;
	...
	connectButton.addListener(this, &ofxLaser::connectButtonPressed);
	...
	pps.addListener(this, &ofxLaser::roundPPS);
